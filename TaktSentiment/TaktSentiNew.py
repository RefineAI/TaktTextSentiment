...
class SentimentIntensityAnalyzer(object):
    ...
    def getItemsYT(self):
        comments = []
        with open("symphony.json") as json_data:
        #with open("Hk3oJpjT-43-YT") as json_data:
            ...
            for i in d:
                #print(str(j) + " ")
                #print(d[j]['text'])
                comments.append(d[j]['text'])
                j = j + 1
        return comments

    def getItemsRAI(self):
        comments = []
        with open("nivea-35s.json") as json_data:
            d = json.load(json_data)
            json_data.close()
            keys = ["likeMost", "likeLeast"]
            #print (len(d))
            for i, entry in enumerate(d):
                #print(entry)
                for key in keys:
                    item = entry["feedback"][key]
                    i = 0
                    #print("item: " + item)
...
if __name__ == '__main__':
    ...
    neutral = []
    compound = []
    #ofileYT = open("symphony.csv", "w")
    ofileAI = open("nivea-35s.csv", "w")

    #writer1 = csv.writer(ofileYT, delimiter=",")
    writer2 = csv.writer(ofileAI, delimiter=",")


    analyzer = SentimentIntensityAnalyzer()
    #items = analyzer.getItemsYT()
    items = analyzer.getItemsRAI()
